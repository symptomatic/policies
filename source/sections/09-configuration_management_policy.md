...
Symptomatic standardizes and automates configuration management through the use of Chef/Salt scripts as well as documentation of all changes to production systems and networks. Chef and Salt automatically configure all Symptomatic systems according to established and tested policies, and are used as part of our Disaster Recovery plan and process.
...
2. No systems are deployed into Symptomatic environments without approval of the Symptomatic CTO.
3. All changes to production systems, network devices, and firewalls are approved by the Symptomatic CTO before they are implemented to assure they comply with business and security requirements.
...
6. Tooling to generate an up-to-date inventory of systems, including corresponding architecture diagrams for related products and services, is hosted on GitLab.
   ...
   * These scripts are used to generate the diagrams and asset lists required by the Risk Assessment phase of Symptomatic's Risk Management procedures ([§4.3.1](#4.3-risk-management-procedures)).
...
10. Symptomatic utilizes development and staging environments that mirror production to assure proper function.
11. Symptomatic also deploys environments locally using Vagrant to assure functionality before moving to staging or production.
12. All formal change requests require unique ID and authentication.
13. Symptomatic uses the [Security Technical Implementation Guides (STIGs)](http://iase.disa.mil/stigs/) published by the Defense Information Systems Agency as a baseline for hardening systems.
...
3. Critical infrastructure services such as logging, monitoring, LDAP servers, or Windows Domain Controllers must be configured with appropriate Salt states.
   * These Salt states have been approved by the VP Engineering and CTO to be in accordance with all Symptomatic policies, including setting appropriate:
...
1. Symptomatic uses automated tooling to ensure systems are up-to-date with the latest security patches.
...
5. If the feature or defect interacts with ePHI, or controls access to data potentially containing ePHI, the code changes must be reviewed by two members of Symptomatic's Blue Team (software security team) before the feature is marked as complete.
   * The Blue Team members will provide a security analysis of features to ensure they satisfy Symptomatic's compliance and security commitments.
   ...
   * Blue Team members are required to undergo annual training on identifying the most common software vulnerabilities and will receive ongoing training on Symptomatic's compliance and security requirements.
